...
class JsonConverter {
 public:
  ...
  //======================================================
  static RemoveMessage parseRemove(const JSON &jrvm) {
	  Song song = parseSong(jrvm[MESSAGE_SONG]);
	  return RemoveMessage(song);
  }
  static RemoveResponseMessage parseRemoveResponse(const JSON &jrvmr) {
    RemoveMessage remove = parseRemove(jrvmr[MESSAGE_REMOVE]);
	std::string status = jrvmr[MESSAGE_STATUS];
	std::string info = jrvmr[MESSAGE_INFO];
